...
{
    std::mutex                            lock;
    std::map<std::string, weak_cgemm_t>   cache;
    std::map<std::string, shared_cgemm_t> speedup;
};

inline znn_cgemm_cache_t znn_cgemm_cache;

inline shared_cgemm_t
get_znn_cgemm(int_t ArCr, int_t AcBr, int_t BcCc, int_t LDA, int_t LDB,
              int_t LDC, int_t alpha, int_t beta, bool Apf, bool Bpf, bool Cpf,
              bool Apf0 = true, bool Bpf0 = true, bool Cpf0 = false,
              bool Cscatter = false, int_t SC_LD0 = 0, int_t SC_LD1 = 0)

{

    ZNN_JIT_ASSERT(LDA % 32 == 0);
    ZNN_JIT_ASSERT(LDB % 32 == 0);
    ZNN_JIT_ASSERT(LDC % 32 == 0);
...
#if defined(ZNN_AVX2)
    name = "znn_cgemm_avx2_";
#elif defined(ZNN_AVX512)
    name = "znn_cgemm_avx512_";
#endif
    ...
    auto fn_code =
        znn_cgemm(ArCr, AcBr, BcCc, LDA * 4, LDB * 4, LDC * 4, alpha, beta, Apf,
                  Bpf, Cpf, Apf0, Bpf0, Cpf0, Cscatter, SC_LD0 * 4, SC_LD1 * 4);

    {
        std::unique_lock<std::mutex> guard(znn_cgemm_cache.lock);

        if (znn_cgemm_cache.speedup.size() > 765)
        {
            znn_cgemm_cache.speedup.clear();
        }

        if (auto ret = znn_cgemm_cache.cache[name].lock(); ret)
        {
        ...
        auto ret = shared_cgemm_t(reinterpret_cast<znn_cgemm_t>(myfn), myso);

        znn_cgemm_cache.speedup[name] = ret;
        znn_cgemm_cache.cache[name]   = ret;
