...
#include "znn/win/input_transform/formula.hpp"
...
#pragma unroll(H_TS)
            ...
            {
                transform_image_1d<M_W, R_W, 1, W_STRIDE / CACHELINE_SIZE>(
                    buffer2 + d * H_TS * W_TS + h * W_TS,
...
#pragma unroll(W_TS)
            ...
            {
                transform_image_1d_last<M_H, R_H, OS, W_TS, W_TS>(
                    out, buffer2 + w, w);
            }
        }
        else
...
#pragma unroll(W_TS)
                for (long_t w = 0; w < W_TS; ++w)
                {
                    transform_image_1d<M_H, R_H, W_TS, W_TS>(
                        buffer1 + d * H_TS * W_TS + w,
                        buffer2 + d * H_TS * W_TS + w);
                }
            }
...
#pragma unroll(W_TS)
                for (long_t w = 0; w < W_TS; ++w)
                {
                    transform_image_1d_last<M_D, R_D, OS, W_TS * H_TS,
                                            W_TS * H_TS>(
