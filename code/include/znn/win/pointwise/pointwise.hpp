...
private:
                           ...
                           long_t r_len)
    {
        ...
        else
        {
            ...
            // else if (n_len > num_threads * 7) // try dynamic value for 9
            // {
            //     std::cout << "NLEN: " << n_len << " NUMTHREADS: " <<
            //     num_threads
            //               << " CLEN: " << c_len << " r_LEN: " << r_len <<
            //               "\n";
    ...
    {
        STRONG_ASSERT((n_len >= n_way) && (num_threads >= n_way) &&
                      (num_threads % n_way == 0));
    ...
    {
        STRONG_ASSERT((n_len >= n_way) && (num_threads >= n_way) &&
                      (n_len % n_way == 0) && (num_threads % n_way == 0));
    ...
    void col_split_schedule(long_t n_way, long_t thread_from,
                            ...
                            long_t c0, long_t c_len, long_t r0, long_t r_len)
    {
        STRONG_ASSERT((c_len >= n_way) && (num_threads >= n_way) &&
                      (c_len % n_way == 0) && (num_threads % n_way == 0));
    ...
    void row_split_schedule(long_t n_way, long_t thread_from,
                            ...
                            long_t c0, long_t c_len, long_t r0, long_t r_len)
    {
        STRONG_ASSERT((r_len >= n_way) && (num_threads >= n_way) &&
                      (num_threads % n_way == 0));
...
public:
    ...
    long_t flops() const
    {
        return matrices::As::num_matrices * matrices::As::num_rows *
               matrices::As::num_cols * matrices::Cs::num_cols * 2;
    }
