...
struct input_transform_t
{
    ...
    {
        return matrices::offset(x[1] * CACHELINE_SIZE,
                                x[0] * num_tiles.prod() +
...
struct output_transform_t
{
    ...
    static constexpr long_t flat_tile_stride = t_size.prod() * CACHELINE_SIZE;
    ...
    static constexpr long_t buffer_floats =
        flat_channel_stride * (size[1] / CACHELINE_SIZE);
...
struct kernel_transform_t
{
    ...
    static constexpr vec3i stride =
        vec3i{k_size[1] * k_size[2] * CACHELINE_SIZE,
              k_size[2] * CACHELINE_SIZE, static_cast<long_t>(CACHELINE_SIZE)};

    static constexpr long_t input_channels  = layer::in_size[1];
    static constexpr long_t output_channels = layer::out_size[1];

    static constexpr long_t vofm_stride = k_size.prod() * CACHELINE_SIZE;
    ...
    {
        return matrices::offset(vofm * CACHELINE_SIZE, ifm, 0);
    }
};

template <class InSize, class InStride, class OutSize, class OutStride,
          class MSize, class KSize, long_t RowBlock = 16, long_t MaxK = 256,
...
struct layer_t
{
    ...
    static constexpr auto m_size     = MSize::value;
    static constexpr auto k_size     = KSize::value;
    static constexpr auto t_size     = m_size + k_size - vec3i::one;
    ...
    static_assert(out_size[1] % CACHELINE_SIZE == 0);

    static constexpr auto num_tiles =
        (subvec<2, 3>(in_size) - k_size + vec3i::one) / m_size;

    using matrices =
        matrices_t<t_size.prod(), num_tiles.prod() * in_size[0], in_size[1],
                   out_size[1], RowBlock, MaxK, MaxKTimesN>;
