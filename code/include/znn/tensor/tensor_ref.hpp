...
#include "znn/tensor/pointer.hpp"
...
protected:
    zi2::vl::vec<long_t, D>     extents_;
    zi2::vl::vec<long_t, D + 1> strides_;
    TPtr                        ptr_ = nullptr;
...
public:
    ...
    explicit const_tensor_ref(TPtr t, zi2::vl::vec<long_t, D> const& e)
        : extents_(e)
        , ptr_(t)
    {
        compute_strides();
    ...
    explicit const_tensor_ref(TPtr t, Args&&... args)
        : extents_(zi2::vl::vec<long_t, D>(std::forward<Args>(args)...))
        , ptr_(t)
    {
        compute_strides();
    }
    ...
    const_reference operator[](long_t i) const noexcept
    {
        return super_type::access(
            znn::detail::tensor::type_t<const_reference>(), i, ptr_,
            extents_.data(), strides_.data());
    }

    zi2::vl::vec<long_t, D> const& shape_vec() const { return extents_; }
    ...
    zi2::vl::vec<long_t, D> const& size() const { return extents_; }
};
...
public:
    explicit tensor_ref(T* t, zi2::vl::vec<long_t, D> const& e)
        : super_type(t, e)
    {
    }
    ...
    {
        return super_type::access(znn::detail::tensor::type_t<reference>(), i,
