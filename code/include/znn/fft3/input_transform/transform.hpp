...
#include "znn/fft3/input_transform/image_transform.hpp"
#include "znn/fft3/kernel_transform/kernel_transform.hpp"
#include "znn/fft3/layer.hpp"
#include "znn/util/kernel_launcher.hpp"
#include <tuple>

namespace znn::fft3::input_transform
{
...
private:
                           ...
                           std::vector<std::pair<long_t, long_t>> const& ts)
    {
        ...
        {
            long_t next = (static_cast<std::size_t>(t) < ts.size() - 1)
                              ? ts[t + 1].first
                              : ts[t].first;
    ...
    using in_transform_fn =
        image_fft<problem::t_size[0], problem::t_size[1], problem::t_size[2],
                  ...
                  problem::matrices::submatrix_stride>;
                           ...
                           std::vector<std::pair<long_t, long_t>> const& ts)
    {
        for (long_t t = t_from; t < t_from + t_len; ++t)
        {
            ...
            individual_ker[thread].push_back({ts[t].first, ts[t].second, next});
        }
    }
    ...
    using kernel_transform_fn =
        kernel_fft<kproblem::k_size[0], kproblem::k_size[1],
                   ...
                   kproblem::matrices::submatrix_stride>;
...
public:
    double input_transform_gbytes() const
    {
        ...
        bytes += problem::size[0] * problem::size[1] *
                 problem::fft_tile_size.prod() * problem::num_tiles.prod() * 3;
        bytes *= 4;
    ...
    double kernel_transform_gbytes() const
    {
        ...
        if constexpr (TransformKernels)
        {
            bytes +=
                ...
                (kproblem::k_size.prod() + kproblem::fft_tile_size.prod() * 3);
        }
                ...
                if constexpr (TransformKernels)
                {
                    ...
                    for (auto const& e : this->individual_ker[i])
                    {
                        kernel_transform_fn::forward(
                            this->kin_ + std::get<0>(e),
                            this->kout_ + std::get<1>(e),
                            reinterpret_cast<float*>(tmp),
                            this->kin_ + std::get<2>(e));
                    }
                }
            };
        }
