...
private:
    ...
    static constexpr long_t subtile_stride =
        Layer::output_transform::flat_subtile_stride;

    void schedule_serial(long_t thread, long_t n0, long_t n_len, long_t c0,
                         long_t c_len, long_t r0, long_t r_len, long_t s0,
                         long_t s_len)
    {
        ...
        znn::jit::shared_gemm_t first =
            Bs::num_row_tiles == 1
                ? znn::jit::get_znn_gemm(
                ...
                : znn::jit::get_znn_gemm(As::tile_row_size, As::tile_col_size,
                                         Bs::tile_col_size, As::row_stride,
                                         Bs::row_stride, Cs::row_stride, 1, 0,
                                         1, 0, 1, al1_pf_, bl1_pf_);
        znn::jit::shared_gemm_t mid = znn::jit::get_znn_gemm(
            As::tile_row_size, As::tile_col_size, Bs::tile_col_size,
            As::row_stride, Bs::row_stride, Cs::row_stride, 1, 1, 1, 0, 1,
            al1_pf_, bl1_pf_);

        znn::jit::shared_gemm_t last =
            Bs::num_row_tiles == 1
                ? znn::jit::get_znn_gemm(
                      As::tile_row_size, As::tile_col_size, Bs::tile_col_size,
                      As::row_stride, Bs::row_stride, Cs::row_stride, 1, 0, 1,
        ...
        allfns_[thread].push_back([=]() {
            ...
            {
                for (long_t s = s0; s < s0 + s_len; ++s)
                {
                    for (long_t c = 0; c < c_len; ++c)
                    {
                        long_t AX = As::submatrix_stride * s; //(2 - s);
                        long_t BX = Bs::submatrix_stride * s;
                        long_t CX = Cs::submatrix_stride * s;
                        // long_t CX_Scatter = subtile_stride * s;
                        long_t CX_Scatter = SIMD_WIDTH * s;

                        // first
                        for (long_t r = 0; r < r_len; ++r)
                        {
                            first(a_ + As::tile_offset(0, r0 + r, n0 + n) + AX,
                                  b_ + Bs::tile_offset(c0 + c, 0, n0 + n) + BX,
                            ...
                            for (long_t r = 0; r < r_len; ++r)
                            {
                                mid(a_ + As::tile_offset(k, r0 + r, n0 + n) +
                                    ...
                                    b_ + Bs::tile_offset(c0 + c, k, n0 + n) +
                                        BX,
                                    c_ +
                                        Cs::tile_offset(c0 + c, r0 + r,
                                                        n0 + n) +
                                        CX,
                                    a_ +
                                        As::tile_offset(k, r0 + r + 1, n0 + n) +
                                        AX,
                                    nullptr,
                                    c_ +
                                        Cs::tile_offset(c0 + c, r0 + r + 1,
    ...
    void schedule_parallel(long_t thread_from, long_t num_threads, long_t n0,
                           ...
                           long_t r_len, long_t s0, long_t s_len)
    {
        // std::cout << "CALL TO SCHEDULE PARALLEL: " << thread_from << ":"
        //           << num_threads << " :: " << n0 << ":" << n_len << " :: " <<
        //           c0
        //           << ":" << c_len << " :: " << r0 << ":" << r_len << "\n";

        if (num_threads == 1)
        {
            schedule_serial(thread_from, n0, n_len, c0, c_len, r0, r_len, s0,
                            s_len);
        }
        else
        {
            ...
            else if ((num_threads % 3 == 0) && (s_len == 3))
            {
                long_t third = num_threads / 3;
                schedule_parallel(thread_from, third, n0, n_len, c0, c_len, r0,
                                  r_len, 0, 1);
                schedule_parallel(thread_from + third, third, n0, n_len, c0,
                                  c_len, r0, r_len, 1, 1);
                schedule_parallel(thread_from + third * 2, third, n0, n_len, c0,
                                  c_len, r0, r_len, 2, 1);
            }
            else if (n_way <= n_len)
            {
            ...
            // {
            //      schedule_parallel_simple(thread_from, num_threads, n0,
            //      n_len,
            //                               c0, c_len, r0, r_len, s0, s_len);
            //      return;
            // }
    ...
    void num_split_schedule_with_tail(long_t n_way, long_t thread_from,
                                      ...
                                      long_t r0, long_t r_len, long_t s0,
                                      long_t s_len)
    {
        STRONG_ASSERT(n_len >= n_way);
        STRONG_ASSERT(num_threads >= n_way);
        ...
        long_t sub_threads = num_threads / n_way;
        long_t sub_n       = n_len / n_way;

        for (long_t i = 0; i < n_way; ++i)
        {
            schedule_parallel(thread_from + i * sub_threads, sub_threads,
                              n0 + i * sub_n, sub_n, c0, c_len, r0, r_len, s0,
                              s_len);
        }

        if (n_len % n_way)
        {
            STRONG_ASSERT((n_len - (n_len % n_way)) == (sub_n * n_way));
            schedule_parallel(thread_from, num_threads, n0 + sub_n * n_way,
                              n_len % n_way, c0, c_len, r0, r_len, s0, s_len);
        }
    }
                            ...
                            long_t s0, long_t s_len)
    {
        ...
        {
            schedule_parallel(thread_from + i * sub_threads, sub_threads, n0,
                              n_len, c0, c_len, r0 + (len + 1) * i, len + 1, s0,
                              s_len);
        }
        for (; i < n_way; ++i)
        {
...
public:
    long_t flops() const
    {
        return matrices::As::num_matrices * matrices::As::num_rows *
               matrices::As::num_cols * matrices::Cs::num_cols * 2 * 3;
    }
        ...
        , bl1_pf_(bpf1)
    {
        schedule_parallel(
            ...
            matrices::Cs::num_col_tiles, 0, matrices::Cs::num_row_tiles, 0, 3);
