...
template <long_t D, long_t H, long_t W, long_t DS, long_t HS, long_t WS,
          long_t KD, long_t KH, long_t KW, long_t PF>
struct output_fft
{
            ...
            for (long_t w = 0; w < W_TS; ++w)
            {
                if constexpr (D_TS > 1)
                {
                    c2cb<D_TS, KD - 1, H_TS * W_TS * 2>(
                        reinterpret_cast<SIMD_FLOAT*>(in) + (h * W_TS + w) * 2,
                        buffer + (h * W_TS + w) * 2);
                }
                // #pragma unroll(D_TS)
                //                 for (long_t d = 0; d < D_TS; ++d)
                //                 {
                //                     SIMD_PREFETCH_L2(reinterpret_cast<SIMD_FLOAT*>(in
                //                     + PF) +
                //                                      (d * W_TS * H_TS + h *
                //                                      W_TS + w) * 2);
                //                 }
...
#pragma unroll(4)
            for (long_t w = 0; w < W_TS; ++w)
            {
                ...
                static_assert(H_TS > 1,
                              "Transform along H has to be greater than 1");
                if constexpr (D_TS > 1)
                {
                ...
                {
                    c2cb<H_TS, KH - 1, W_TS * 2>(
                        reinterpret_cast<SIMD_FLOAT*>(in) +
                            (d * H_TS * W_TS + w) * 2,
                        buffer + (d * H_TS * W_TS + w) * 2);
                }
                if constexpr (false && D_TS == 1)
                {
#pragma unroll(H_TS)
                    for (long_t h = 0; h < H_TS; ++h)
                    {
                        SIMD_PREFETCH_L2(
                            reinterpret_cast<SIMD_FLOAT*>(in + PF) +
                            (d * W_TS * H_TS + h * W_TS + w) * 2);
                        SIMD_PREFETCH_L2(
                            reinterpret_cast<SIMD_FLOAT*>(in + PF) +
                            (d * W_TS * H_TS + h * W_TS + w) * 2 + 1);
                    }
                }
            }
        }
...
#pragma unroll(4)
            ...
            {
                // r2cb<W + KW - 1, KW - 1, 1, H_TS * 2>(
                //    buffer,
                //    reinterpret_cast<SIMD_FLOAT*>(in) +
                //        (d * H_TS * W_TS + h) * 2);
                r2cb<W + KW - 1, KW - 1, 1, 2>(
                    buffer, buffer + (d * H_TS * W_TS + h * W_TS) * 2);
#pragma unroll(W)
                for (long_t w = 0; w < W; ++w)
                {
                    buffer[w] = buffer[w] * svec;
                    SIMD_STREAM(out + d * DS + h * HS + w * WS, buffer[w]);
                }
            }
        }
    }
