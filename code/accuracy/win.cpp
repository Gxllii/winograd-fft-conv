                                 ...
                                 std::string const& weights = "")
{
    ...
    direct_corr_parallel<B, C1, D, H, W, C2, k_size::value[0], k_size::value[1],
...
template <long_t B, long_t C1, long_t C2, long_t ID, long_t IH, long_t IW,
          class k_size, class m_size, long_t Cores = ZNN_NUM_CORES,
          long_t HT = 1, long_t RB = 16>
measurement measure_error_winograd(std::string const& name,
                                   std::string const& weights = "")
{

    std::cout << "WINOGRAD CONVOLUTION ERROR MEASUREMENT FOR: " << name
              << std::endl;

    // znn::win::avx512::clear_znn_gemms();

    static constexpr long_t D =
        padded_size(ID, m_size::value[0], k_size::value[0]);
    static constexpr long_t H =
        padded_size(IH, m_size::value[1], k_size::value[1]);
    static constexpr long_t W =
        padded_size(IW, m_size::value[2], k_size::value[2]);
    ...
    using layer = layer_t<idim, istrides, odim, ostrides, m_size, k_size, RB>;
    ...
    std::cout << " DONE\n"
              << "Computing using winograd..." << std::flush;
...
template <class M>
measurement measure_vgg_winograd(bool inference = false)
{
    using K = vek<1, 3, 3>;

    std::cout << "F(" << M::value[1] << "x" << M::value[2] << ", 3x3)"
              << std::endl;
    auto m = measure_error_winograd<1, 64, 64, 1, 226, 226, K, M>(
        "vgg 1.2", inference ? "./trained_kernels/vgg-c1_2" : "");

    m = merge_statistics(
        m, measure_error_winograd<1, 64, 128, 1, 114, 114, K, M>(
               "vgg 2.1", inference ? "./trained_kernels/vgg-c2_1" : ""));

    m = merge_statistics(
        m, measure_error_winograd<1, 128, 128, 1, 114, 114, K, M>(
               "vgg 2.2", inference ? "./trained_kernels/vgg-c2_2" : ""));

    m = merge_statistics(
        m, measure_error_winograd<1, 128, 256, 1, 58, 58, K, M>(
               "vgg 3.1", inference ? "./trained_kernels/vgg-c3_1" : ""));

    m = merge_statistics(
        m, measure_error_winograd<1, 256, 256, 1, 58, 58, K, M>(
               "vgg 3.2", inference ? "./trained_kernels/vgg-c3_2" : ""));

    m = merge_statistics(
        m, measure_error_winograd<1, 256, 512, 1, 30, 30, K, M>(
               "vgg 4.1", inference ? "./trained_kernels/vgg-c4_1" : ""));

    m = merge_statistics(
        m, measure_error_winograd<1, 512, 512, 1, 30, 30, K, M>(
               "vgg 4.2", inference ? "./trained_kernels/vgg-c4_2" : ""));

    m = merge_statistics(
        m, measure_error_winograd<1, 512, 512, 1, 16, 16, K, M>(
...
template <class M>
measurement measure_c3d_winograd(bool inference = false)
{
    using K = vek<3, 3, 3>;

    std::cout << "F(" << M::value[0] << "x" << M::value[1] << "x" << M::value[2]
              << ", 3x3x3)" << std::endl;
    auto m = measure_error_winograd<1, 64, 128, 18, 58, 58, K, M>(
        "c3d C2a", inference ? "./trained_kernels/C3D-c2a" : "");

    m = merge_statistics(
        m, measure_error_winograd<1, 128, 256, 10, 30, 30, K, M>(
               "c3d C3a", inference ? "./trained_kernels/C3D-c3a" : ""));

    m = merge_statistics(
        m, measure_error_winograd<1, 256, 256, 10, 30, 30, K, M>(
               "c3d C3b", inference ? "./trained_kernels/C3D-c3b" : ""));

    m = merge_statistics(
        m, measure_error_winograd<1, 256, 512, 6, 16, 16, K, M>(
               "c3d C4a", inference ? "./trained_kernels/C3D-c4a" : ""));

    m = merge_statistics(
        m, measure_error_winograd<1, 512, 512, 6, 16, 16, K, M>(
...
int main()
{
    ...
    // clang-format off
    c3ds << "+-----------+--------------+--------------+--------------+--------------+--------------+--------------+\n";
    c3ds << "| C3D       |    DIRECT    | F(2x2x2,3^3) | F(4x4x4,3^3) | F(4x6x6,3^3) | F(6x6x6,3^3) | F(8x6x6,3^3) |\n";
    c3ds << "+-----------+--------------+--------------+--------------+--------------+--------------+--------------+\n";

    vggs << "+-----------+--------------+--------------+--------------+--------------+--------------+--------------+\n";
    vggs << "| VGG       |    DIRECT    |  F(2x2,3x3)  |  F(4x4,3x3)  |  F(5x5,3x3)  |  F(6x6,3x3)  |  F(8x8,3x3)  |\n";
    vggs << "+-----------+--------------+--------------+--------------+--------------+--------------+--------------+\n";
...

    {
        ...
        r[1] = measure_vgg_winograd<vek<1, 2, 2>>(false);
        r[2] = measure_vgg_winograd<vek<1, 4, 4>>(false);
        r[3] = measure_vgg_winograd<vek<1, 5, 5>>(false);
        r[4] = measure_vgg_winograd<vek<1, 6, 6>>(false);
        r[5] = measure_vgg_winograd<vek<1, 8, 8>>(false);
        ...
        r[1] = measure_vgg_winograd<vek<1, 2, 2>>(true);
        r[2] = measure_vgg_winograd<vek<1, 4, 4>>(true);
        r[3] = measure_vgg_winograd<vek<1, 5, 5>>(true);
        r[4] = measure_vgg_winograd<vek<1, 6, 6>>(true);
        r[5] = measure_vgg_winograd<vek<1, 8, 8>>(true);
...

    {
        ...
        r[1] = measure_c3d_winograd<vek<2, 2, 2>>(false);
        r[2] = measure_c3d_winograd<vek<4, 4, 4>>(false);
        r[3] = measure_c3d_winograd<vek<4, 6, 6>>(false);
        r[4] = measure_c3d_winograd<vek<6, 6, 6>>(false);
        r[5] = measure_c3d_winograd<vek<8, 6, 6>>(false);
        ...
        r[1] = measure_c3d_winograd<vek<2, 2, 2>>(true);
        r[2] = measure_c3d_winograd<vek<4, 4, 4>>(true);
        r[3] = measure_c3d_winograd<vek<4, 6, 6>>(true);
        r[4] = measure_c3d_winograd<vek<6, 6, 6>>(true);
        r[5] = measure_c3d_winograd<vek<8, 6, 6>>(true);
